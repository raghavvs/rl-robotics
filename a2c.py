...
def get_screen():
    ...
    screen = envs._get_observation().transpose((2, 0, 1))
...
def train(envs, policy, optimizer, season, tmax, batch_size, epsilon, beta, opt_epoch, writer, discount=0.993, epsilon_decay=0.999, beta_decay=0.998):
    ...
    return save_scores, best_mean_reward

def main():
    # Set up the environment
    envs = KukaDiverseObjectEnv(renders=False, isDiscrete=False, removeHeightHack=False, maxSteps=20, isTest=True)
    envs.cid = p.connect(p.DIRECT)
    ...
    season = 1000000
    tmax = 1000 // num_agents
    batch_size = 128
    epsilon = 0.07
    beta = .01
    opt_epoch = 10
    start_time = timeit.default_timer()
    save_scores, best_mean_reward = train(envs, policy, optimizer, season, tmax, batch_size, epsilon, beta, opt_epoch, writer)
